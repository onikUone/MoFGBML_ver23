package jmetal6.qualityindicator.impl.hypervolume;
...
import jmetal6.util.solutionattribute.impl.HypervolumeContributionAttribute;
...
public class WFGHypervolume<S extends Solution<?>> extends Hypervolume<S> {
   ...
   * @throws FileNotFoundException
   */
  public WFGHypervolume(jmetal6.util.front.Front referenceParetoFront) {
   ...
   */
  private double hypervolume(jmetal6.util.front.Front front, jmetal6.util.front.Front referenceFront) {

    jmetal6.util.front.Front invertedFront;
  ...
  public List<S> computeHypervolumeContribution(List<S> solutionList, List<S> referenceFrontList) {
    if (solutionList.size() > 1) {
      jmetal6.util.front.Front front = new ArrayFront(solutionList);
      jmetal6.util.front.Front referenceFront = new ArrayFront(referenceFrontList);
      ...
      jmetal6.util.front.Front normalizedFront = frontNormalizer.normalize(front);
      ...
      jmetal6.util.front.Front invertedFront = FrontUtils.getInvertedFront(normalizedFront);

      // shift away from origin, so that boundary points also get a contribution > 0
      for (int i = 0; i < invertedFront.getNumberOfPoints(); i++) {
        jmetal6.util.point.Point point = invertedFront.getPoint(i);
